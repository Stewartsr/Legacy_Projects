...
const char *WINDOWTITLE = { "Particle System -- Stewart Rodger" };
...
Animate( void )
{
	...
	{
		dt = Time - p->t0;
		//p->x = p->x + 0.1*p->vx*dt; // Black/white hole at origin
		//p->y = p->y + 0.1*p->vy*dt;
		//p->z = p->z + 0.1*p->vz*dt;
		//p->x = p->x + 0.1*Time; //fireball but square
		//p->y = p->y + 0.1*Time;
		//p->z = p->z + 0.1*Time;
		p->x = p->x + Ranf(-1., 6)*Time; //fireball
		p->y = p->y + Ranf(-1., 6)*Time;
		p->z = p->z + Ranf(-1., 6)*Time;
