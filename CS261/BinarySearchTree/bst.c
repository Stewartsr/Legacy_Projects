/*
  File: bst.c
  Implementation of the binary search tree data structure that can store any arbitrary struct in its nodes.
  Programer: Stewart Rodger
  Date: 2/27/2017
...
bool containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    struct Node *this = tree->root;
    while (this != 0){
        if(val == this->val){
	     return 1; } //returns true if the value is found
	if (val < this->val) {
	     this = this->left; }
	else { 
             this = this->right;
