...
const char *WINDOWTITLE = { "Final Project -- Stewart Rodger" };
...
MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort && isSun)
			{
				p->s = (lng + M_PI) / (2.*M_PI) + 0.01*sin(warp);
				p->t = (lat + M_PI / 2.) / M_PI + 0.01*warp;
			}
			else
			{
				p->s = (lng + M_PI) / (2.*M_PI);
...
void SetTexture(int a, int b) {
	...
	Texture2 = BmpToTexture("earth_daymap.bmp", &width, &height);
	...
	Texture3 = BmpToTexture("moon.bmp", &width, &height);
	glBindTexture(GL_TEXTURE_2D, tex2); // designate the tex1 texture as the “current texture”
										// and set its parameters
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
Display( )
{
	...
	if (theView == 0) {
		gluLookAt(1, 15, 1, 0., 0., 0., 0., 1., 0.);
	}
	if (theView == 1) {
		gluLookAt(7, 7, 7, 0., 0., 0., 0., 1., 0.);
	}
	if (theView == 2) {
		glRotatef(-180, 0., 1, 0.); //rotate once
		gluLookAt(1, 0, 0,		0., 0., 0.,		0., 1., 0);
		glRotatef(spins, 0., 1, 0.); //rotate
		glTranslatef(11, 0, 0);
		glRotatef(-.4*spins, 0., 1, 0.); //orbit
	}
	if (theView == 3) {
		glRotatef(23.5, 1., 0., 1.); //erf tilt
		glRotatef(-180, 0., 1, 0.); //rotate once
		gluLookAt(1, 0, 0, 0., 0., 0., 0., 1., 0);
		glRotatef(-spins, 0., 1, 0.); //rotate
		glTranslatef(8, 0, 0);
	...
	//Stationary point light and sphere to show where it is (White)
...
//Set spotlights to light up that damn sun
	SetSpotLight(GL_LIGHT1, 0, 8, 0,	0, -8, 0,	1, 1, 1);
	glEnable(GL_LIGHT1);
	SetSpotLight(GL_LIGHT2, 0, -8, 0,	0, 8, 0,	1, 1, 1);
	glEnable(GL_LIGHT2);
	SetSpotLight(GL_LIGHT3, 8, 0, 0,	-8, 0, 0,	1, 1, 1);
	glEnable(GL_LIGHT3);
	SetSpotLight(GL_LIGHT4, -8, 0, 0,	8, 0, 0,	1, 1, 1);
	glEnable(GL_LIGHT4);
	SetSpotLight(GL_LIGHT5, 0, 0, -8,	0, 0, 8,	1, 1, 1);
	glEnable(GL_LIGHT5);
	SetSpotLight(GL_LIGHT6, 0, 0, 8,	0, 0, -8,	1, 1, 1);
	glEnable(GL_LIGHT6);
	...
	//Mjb Sphere goes here instead of teapot
	isSun = true;
	MjbSphere(3., 30, 30);
	isSun = false;
	...
	glShadeModel(GL_SMOOTH);
	glPushMatrix();
	glRotatef(0.4*spins, 0., 1., 0.); //erf orbit
	glTranslatef(-11, 0., 0);
	
	// earth
	glRotatef(23.5, 1., 0., 1.); //erf tilt
	glRotatef(spins, 0., 1, 0.); //erf rotate
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, tex1);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	MjbSphere(1, 30, 30); //erf
	glRotatef(spins, 0., 1, 0.); //mun orbit
	glTranslatef(2, 0., 0);
	glDisable(GL_TEXTURE_2D);
	//erf chunk end
	...
	//mun chunk
	SetMaterial(0, 0, 0, 3);
	glRotatef(spins, 0., 1, 0.); //mun rotation
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, tex2);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	MjbSphere(.3, 30, 30); //Mun
	glDisable(GL_TEXTURE_2D);
	glTranslatef(11, 0., 0);
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Final Project - rodgers" );
